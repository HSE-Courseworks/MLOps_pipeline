...
def predict_text(text=Body(embed=True)):
    try:
        if not text:
            raise HTTPException(
                status_code=401, detail="No text provided for prediction."
            )
        ...
        try:
            if True:
                model_data = minio_client.get_object(
                    "mlflow",
                    "37/48f742c5bc724f61baa4b49401197361/artifacts/reactions_model/reactions_model.joblib",
                )
                model_bytes = model_data.read()
                model = joblib.load(BytesIO(model_bytes))
        except Exception as e:
            raise HTTPException(
                status_code=502, detail=f"Failed to load model: {str(e)}"
            )
