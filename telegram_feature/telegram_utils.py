...
class TelegramClient:
        ...
        with self.app:
            ...
            for message in messages:
                ...
                if not self.post_exists_in_db(message.id):
                    ...
                    self.cursor.execute("INSERT INTO channels (channel_id, channel_name) VALUES (%s, %s) ON CONFLICT (channel_id) DO NOTHING", (channel_id, channel_name))
                    if (media_group is None or 
                        ...
                        if post_text is not None:
                            ...
                            self.cursor.execute("UPDATE posts SET views = %s WHERE post_id = %s AND channel_id = %s", (message.views, message_id, channel_id))
                            if self.cursor.rowcount == 0:
                                self.cursor.execute("INSERT INTO posts (post_id, channel_id, post_text, views, time) VALUES (%s, %s, %s, %s, %s)", (message_id, channel_id, post_text, message.views, message.date))
                        ...
                        if message.text is not None or message.caption is not None:
                            post_text = message.text if message.text is not None else message.caption
                            self.cursor.execute("UPDATE posts SET post_text = %s WHERE post_id = %s AND channel_id = %s", (post_text, message_id, channel_id))
                    if message.reactions is not None:
                        self.cursor.execute("DELETE FROM reactions WHERE post_id = %s", (message.id,))
                        for reaction in message.reactions.reactions:
                            self.cursor.execute("INSERT INTO reactions (post_id, emoji, count) VALUES (%s, %s, %s)", (message.id, reaction.emoji, reaction.count))
            self.conn.commit()
    ...
    def print_data(self):
        ...
        for post in posts:
            print(f"Post {post[0]}:")
            self.cursor.execute("SELECT channel_id FROM channels WHERE channel_id = %s", (post[1],))
            print(f"Channel: {self.cursor.fetchone()[0]}")
            print(f"Text: {post[2]}")
            print(f"Views: {post[3]}")
            print(f"Time: {post[4]}")

            self.cursor.execute("SELECT media_type, file_id FROM media WHERE post_id = %s", (post[0],))
            ...
            self.cursor.execute("SELECT emoji, count FROM reactions WHERE post_id = %s", (post[0],))
